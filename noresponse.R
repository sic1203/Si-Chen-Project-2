                          ...
                          sidebarLayout(
                            sidebarPanel(
                              ...
                              selectInput("NghSelect",
                                          ...
                                          multiple = TRUE,
                                          selectize = TRUE,
                                          selected=c("Arlington","Allentown","Bloomfiled","East Liberty","East Hills","Banksville")),
                              #Income Select
...
server <- function(input, output, session = session) {
  ...
  output$plot1<-renderPlotly({
    ...
    p1<-plot_ly(dat1,x=~Neighborhood,y=~Estimate..Total....No.Social.Security.income,name="Without SSI",type="bar")
    p2<-plot_ly(dat2,x=~Neighborhood,y=~Estimate..Total.,name = "With SSI",type="bar")
    subplot(p1,p2)
    })
  ...
  output$plot2 <- renderPlotly(({
    ...
    ggplotly(
      ggplot(data =dat3, aes(x=Estimate..Total.))+
        ...
        xlab("Income") +
        ylab("Density") +
        ggtitle("Distribution of Total Income",subtitle = "with social Security Income")
    )
  })
  )
  ...
  output$plot3 <- renderPlotly(({
    ...
    ggplotly(
      ggplot(data =dat4, aes(x=Estimate..Total....No.Social.Security.income))+
        ...
        xlab("Income") +
        ylab("Density") +
        ggtitle("Distribution of Total Income",subtitle = "without Social Security Income")
    )
  })
  )
